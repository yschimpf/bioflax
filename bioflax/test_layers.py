...
class Test_RandomDenseLinearFA(unittest.TestCase):
    ...
    def test_derivative_single_layer(self):
        ...
        # dL/dB = 0
        self.assertTrue((delta_params["params"]["B"] == jnp.zeros((5, 10))).all())
        # dL/dx = B(y-target)
        self.assertTrue(jnp.allclose(delta_x , (params["params"]["B"]@(y-target).T).T))
        # dL/dW = x^T * (y-target)
        self.assertTrue(jnp.allclose(delta_params["params"]["Dense_0"]["kernel"] , x.T@(y-target)))
        # dL/db = y-target
        self.assertTrue(jnp.allclose(delta_params["params"]["Dense_0"]["bias"] , y-target))
    ...
    def test_derivative_multi_layer(self):
        ...
        # dL/dB_2 = 0
        self.assertTrue((delta_params["params"]["RandomDenseLinearFA_1"]["B"] == jnp.zeros((15, 10))).all())
        # dL/dB_1 = 0
        self.assertTrue((delta_params["params"]["RandomDenseLinearFA_0"]["B"] == jnp.zeros((5, 15))).all())
        ...
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearFA_1"]["Dense_0"]["kernel"] , v_1.T@(y-target)))
        # dL/db_2 = y-target
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearFA_1"]["Dense_0"]["bias"] , y-target))
...
class Test_RandomDenseLinearKP(unittest.TestCase):
    ...
    def test_derivative_single_layer(self):
        ...
        # dL/dB = 0
        self.assertTrue((delta_params["params"]["B"] == delta_params["params"]["Dense_0"]["kernel"]).all())
        # dL/dx = B(y-target)
        self.assertTrue(jnp.allclose(delta_x , (params["params"]["B"]@(y-target).T).T))
        # dL/dW = x^T * (y-target)
        self.assertTrue(jnp.allclose(delta_params["params"]["Dense_0"]["kernel"] , x.T@(y-target)))
    ...
    def test_derivative_multi_layer(self):
        ...
        # dL/dW_2 = v_1^T * (y-target)
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearKP_1"]["Dense_0"]["kernel"] , v_1.T@(y-target)))
        # dL/db_2 = y-target
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearKP_1"]["Dense_0"]["bias"] , y-target))
        # dL/dW_1 = x^T * (B_2 * (y-target)^T)^T = x^T * dL/dv_1
...
class Test_RandomDenseLinearDFA(unittest.TestCase):
    ...
    def test_output_single_layer_output(self):
        ...
        params = model.init(jax.random.PRNGKey(0), x)
        y = model.apply(params, x)
        self.assertTrue(jnp.allclose(y, params["params"]["Dense_0"]["bias"]+ x @ params["params"]["Dense_0"]["kernel"]))
    ...
    def test_hidden_derivative_single_layer(self):
        ...
        # which it essentially does here. The derivative makes no semnantic sense.
        self.assertTrue(jnp.allclose(delta_params["params"]["Dense_0"]["bias"] , (params["params"]["B"]@(y-target).T).T))
    ...
    def test_derivative_multi_layer(self):
        ...
        # dL/dB_1 = 0
        self.assertTrue((delta_params["params"]["RandomDenseLinearDFAHidden_0"]["B"] == jnp.zeros((15,10))).all())
        # dL/dx = (y-target)^T
        self.assertTrue(jnp.allclose(delta_x , y-target))
        # dL/dW_2 = v_1^T * (y-target)
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearDFAOutput_0"]["Dense_0"]["kernel"] , v_1.T@(y-target)))
        # dL/db_2 = y-target
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearDFAOutput_0"]["Dense_0"]["bias"] , y-target))
        # dL/dW_1 = x^T * (B_2 * (y-target)^T)^T = x^T * dL/dv_1
    ...
    def test_derivative_multi_layer_activation(self):
        ...
        # dL/dB_2 = 0
        self.assertTrue((delta_params["params"]["RandomDenseLinearDFAHidden_1"]["B"] == jnp.zeros((15,10))).all())
        # dL/dB_1 = 0
        self.assertTrue((delta_params["params"]["RandomDenseLinearDFAHidden_0"]["B"] == jnp.zeros((20,10))).all())
        # dL/dx = (y-target)^T
        self.assertTrue(jnp.allclose(delta_x , y-target))
        # dL/dW_2 = v_1^T * (y-target)
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearDFAOutput_0"]["Dense_0"]["kernel"] , v_2.T@(y-target)))
        # dL/db_2 = y-target
        self.assertTrue(jnp.allclose(delta_params["params"]["RandomDenseLinearDFAOutput_0"]["Dense_0"]["bias"] , y-target))
        # dL/dW_1 = x^T * (B_2 * (y-target)^T)^T = x^T * dL/dv_1
